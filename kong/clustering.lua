...
local gzip_inflate = utils.gzip_inflate
local gzip_deflate = utils.gzip_deflate
...
local function update_config(config_table, update_cache)
  ...
  if update_cache then
    ...
    if not f then
      ...
      local res
      res, err = f:write(assert(gzip_deflate(cjson_encode(config_table))))
      if not res then
...
function _M.handle_cp_websocket()
  ...
  do
    ...
    payload = assert(gzip_deflate(payload))
    ...
    if ngx.worker.id() == 0 then
      ...
      if f then
        ...
        if config then
          ...
          config, err = gzip_inflate(config)
          if config then
